-- ScoopHub V3.6 GUI library (Full Header Mobile Drag + 70% Scale)
...
local Library={Unloaded=false,Build="SCOOPHUB_V3_6_FULL_HEADER_DRAG"}
...
function Library:CreateWindow(cfg)
 ...
 local function resize()
  ...
  if mobile then
   -- Same strategy as the known mobile-supported ScoopHub reference:
   -- shrink the normal responsive GUI by an additional percentage.
   Scale.Scale=math.clamp(
    baseScale*MOBILE_GUI_SCALE,
    .35,
    MOBILE_GUI_SCALE
   )
  else
  ...
  -- Only portrait phones need one-column cards.
 ...
 local BrandRow=N("Frame",{
  Name="ScoopHubBrandRow",
  BackgroundTransparency=1,
  BorderSizePixel=0,
  Position=UDim2.new(0,47,0,2),
  Size=UDim2.fromOffset(225,18),
  ZIndex=51
 },Header)
 ...
 BrandPrimary.AutomaticSize=Enum.AutomaticSize.X
 BrandPrimary.LayoutOrder=1
 ...
 BrandPremium.AutomaticSize=Enum.AutomaticSize.X
 BrandPremium.LayoutOrder=2
 ...
 local RawVersionText=
  tostring(cfg.Version or "V1.1"):
   match("^%s*(.-)%s*$")

 -- Version pill is intentionally limited to exactly four characters:
 -- V#.#  (examples: V1.1, V1.5, V2.0)
 local CleanVersionText=
  RawVersionText:
   match("^V%d%.%d$")

 if not CleanVersionText then
  CleanVersionText="V1.1"
 end
 ...
 local DiscordPill=C(N("Frame",{Name="DiscordPill",AnchorPoint=Vector2.new(.5,.5),Position=UDim2.new(.55,0,.5,0),
  ...
  BorderSizePixel=0,ClipsDescendants=true,ZIndex=52},Header),11)
 ...
 local Discord=N("TextButton",{Text="",BackgroundTransparency=1,BorderSizePixel=0,Size=UDim2.fromScale(1,1),AutoButtonColor=false,ZIndex=53},DiscordPill)
 ...
 -- Dedicated drag surface for mouse/touch.
 -- Child labels/images in the branded header can swallow touch input on mobile,
 -- so dragging no longer depends on Header.InputBegan alone.
 local HeaderDragZone=N("TextButton",{
  Name="ScoopHubHeaderDragZone",
  Text="",
 ...
 -- Close confirmation modal. This intentionally mirrors the reference
 -- ScoopHub confirmation layout/animation, while using this library's theme.
 local CloseConfirmHolder=N("Frame",{
  Name="ScoopHubCloseConfirmHolder",
  Active=true,
 ...
 C(CloseConfirmHolder,8)

 local CloseConfirmBox=C(N("Frame",{
  Name="ScoopHubCloseConfirmBox",
  AnchorPoint=Vector2.new(.5,.5),
  ...
  ZIndex=301
 },CloseConfirmHolder),8)
 S(CloseConfirmBox,T.Red,.15,1.5)
 N("UIGradient",{Color=ColorSequence.new({
  ColorSequenceKeypoint.new(0,T.Top),ColorSequenceKeypoint.new(.52,T.Mid),ColorSequenceKeypoint.new(1,T.Low)
 }),Rotation=16},CloseConfirmBox)

 -- Subtle white decorative dots matching the main ScoopHub background.
 -- They stay behind the modal content and do not affect interaction.
 local CloseConfirmDots=N("Frame",{
  Name="ScoopHubCloseConfirmDots",
  Size=UDim2.fromScale(1,1),
  BackgroundTransparency=1,
  BorderSizePixel=0,
  ZIndex=301
 },CloseConfirmBox)
 local closeDotRandom=Random.new((LP.UserId or 0)+137)
 for i=1,18 do
  local dotSize=closeDotRandom:NextNumber()>.82 and 2 or 1
  C(N("Frame",{
   Name="ConfirmDot",
   Position=UDim2.fromScale(closeDotRandom:NextNumber(.035,.965),closeDotRandom:NextNumber(.06,.94)),
   Size=UDim2.fromOffset(dotSize,dotSize),
   BackgroundColor3=Color3.fromRGB(255,255,255),
   BackgroundTransparency=closeDotRandom:NextNumber(.68,.88),
   BorderSizePixel=0,
   ZIndex=301
  },CloseConfirmDots),20)
 end

 local CloseConfirmTitle=label(
  CloseConfirmBox,
  tostring(cfg.CloseConfirmTitle or "Close ScoopHub?"),
  UDim2.new(0,0,0,18),
  UDim2.new(1,0,0,18),
  16,
  T.White,
 ...
 CloseConfirmTitle.Name="ConfirmTitle"
 CloseConfirmTitle.TextXAlignment=Enum.TextXAlignment.Center
 CloseConfirmTitle.TextYAlignment=Enum.TextYAlignment.Center
 CloseConfirmTitle.ZIndex=302

 local CloseConfirmText=label(
  CloseConfirmBox,
  tostring(cfg.CloseConfirmText or "Are you sure you want to close?"),
  UDim2.new(0,0,0,44),
  UDim2.new(1,0,0,16),
  13,
  T.Muted,
 ...
 CloseConfirmText.Name="ConfirmDesc"
 CloseConfirmText.TextXAlignment=Enum.TextXAlignment.Center
 CloseConfirmText.TextYAlignment=Enum.TextYAlignment.Center
 CloseConfirmText.ZIndex=302

 local CloseConfirmCancel=C(N("TextButton",{
  Name="CancelBtn",
  Font=T.Font,
  Text=tostring(cfg.CloseConfirmCancelText or "Cancel"),
  ...
  Size=UDim2.new(.5,-24,0,34),
  AutoButtonColor=false,
  ZIndex=302
 },CloseConfirmBox),6)

 local CloseConfirmAccept=C(N("TextButton",{
  Name="OkBtn",
  Font=T.Font,
  Text=tostring(cfg.CloseConfirmAcceptText or "Close"),
  TextColor3=T.White,
  TextSize=14,
 ...
 local closePromptSerial=0

 local function showClosePrompt()
  if window.Closed or CloseConfirmHolder.Visible then return end
  closePromptSerial+=1
  CloseConfirmHolder.Visible=true
  CloseConfirmHolder.BackgroundTransparency=1
  CloseConfirmBox.Size=UDim2.fromOffset(232,110)
  tw(CloseConfirmHolder,{BackgroundTransparency=.45},.15)
  local openTween=TS:Create(
   CloseConfirmBox,
   TweenInfo.new(.18,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
   {Size=UDim2.fromOffset(250,120)}
  )
  openTween:Play()
 end
 ...
 local function hideClosePrompt()
  if not CloseConfirmHolder.Visible then return end
  closePromptSerial+=1
  local token=closePromptSerial
  tw(CloseConfirmHolder,{BackgroundTransparency=1},.15)
  task.delay(.15,function()
   if token==closePromptSerial and CloseConfirmHolder.Parent then
    CloseConfirmHolder.Visible=false
   end
  end)
 end
 ...
 function window:RequestClose()
  ...
  if cfg.CloseConfirm==false then
   if type(cfg.OnClose)=="function" then pcall(cfg.OnClose) end
   self:Destroy()
   return
  end
  showClosePrompt()
 ...
 own(CloseConfirmCancel.Activated:Connect(function()
  hideClosePrompt()
 end))

 own(CloseConfirmAccept.Activated:Connect(function()
 ...
 function window:AddTab(c)
  ...
  function tab:_build()
   ...
   self.Scroll=N("ScrollingFrame",{Name=name.."Scroll",Position=UDim2.new(0,0,0,24),Size=UDim2.new(1,-8,1,-24),
    ...
    Active=true,
    ScrollBarThickness=mobile and 8 or 4,
   ...
   if mobile then
    ...
    own(self.Scroll.InputBegan:Connect(function(input)
     ...
     then
      ...
      canvasStart=self.Scroll.CanvasPosition.Y
     end
    end))

    own(UIS.InputChanged:Connect(function(input)
     if not touchScrolling
      or input~=touchInput
      or not touchStart
      or not self.Scroll.Parent
     then
      return
     end

     local deltaY=
      input.Position.Y-touchStart.Y

     local visibleHeight=
      self.Scroll.AbsoluteWindowSize.Y
       /math.max(Scale.Scale,.01)

     local maxY=math.max(
      0,
      self.Scroll.CanvasSize.Y.Offset
       -visibleHeight
     )
     ...
     local wanted=
      math.clamp(
       canvasStart
        -(deltaY/math.max(Scale.Scale,.01)),
       0,
       maxY
      )
    ...
    own(UIS.InputEnded:Connect(function(input)
  ...
  function tab:_reflow()
   ...
   -- Mobile needs extra space below the final section so the last
   -- controls can be dragged fully above Roblox's touch/UI area.
   local bottomPadding=
    mobile and 140 or 28
   ...
   -- If the old CanvasPosition is now beyond the valid range after a
   -- resize/reflow, clamp it instead of letting mobile scrolling lock.
   task.defer(function()
    if not self.Scroll
     or not self.Scroll.Parent
    then
    ...
    local maxY=math.max(
     0,
     canvasBottom-visibleHeight
    )

    local currentY=
     self.Scroll.CanvasPosition.Y
  ...
  function tab:AddSection(sc)
   ...
   function sec:AddDropdown(x)
    ...
    local api={}
    local rows={}
    local maxVisibleRows=mobile and 9 or 6
    local rowHeight=mobile and 30 or 34
    ...
    local dropdownTouchInput=nil
    local dropdownTouchStart=nil
    local dropdownCanvasStart=0
    local dropdownTouchMoved=false
    local dropdownSuppressUntil=0
     ...
     own(UIS.InputBegan:Connect(function(input)
      if not pop.Visible
       or input.UserInputType
        ~=Enum.UserInputType.Touch
       or not pointInside(scroll,input.Position)
      then
       return
      end

      dropdownTouchInput=input
      dropdownTouchStart=input.Position
      dropdownCanvasStart=scroll.CanvasPosition.Y
      dropdownTouchMoved=false
     end))

     own(UIS.InputChanged:Connect(function(input)
      if input~=dropdownTouchInput
       or not dropdownTouchStart
       or not pop.Visible
       or not scroll.Parent
      then
       return
      end

      local deltaY=
       input.Position.Y-dropdownTouchStart.Y

      if math.abs(deltaY)>=5 then
       dropdownTouchMoved=true
      end
      ...
      local maxY=math.max(
       0,
       scroll.CanvasSize.Y.Offset-visibleHeight
      )
      ...
      scroll.CanvasPosition=
       Vector2.new(0,wanted)
     end))

     own(UIS.InputEnded:Connect(function(input)
      if input~=dropdownTouchInput then
       return
      end

      if dropdownTouchMoved then
       dropdownSuppressUntil=os.clock()+.18
      end

      dropdownTouchInput=nil
    ...
    local function rebuild()
     ...
     for _,o in ipairs(options) do
      ...
      then
       ...
       own(r.Activated:Connect(function()
        if mobile
         and os.clock()<dropdownSuppressUntil
        then
         return
        end
     ...
     local rowGap=2
     local rowCount=math.max(n,0)

     local logicalContentHeight=
      (rowCount*rowHeight)
     ...
     -- Clamp an old scroll position after search/filter changes.
     task.defer(function()
      ...
      local maxY=math.max(
       0,
       logicalContentHeight-visibleHeight
      )
 ...
 local dh,dm=false,false
 local moved=false
 local sm,sp
 local touch=nil

 local function beginWindowDrag(i)
  if minimized then
   return
  end
 ...
 -- Dedicated drag zone is the primary mobile drag target.
 own(HeaderDragZone.InputBegan:Connect(beginWindowDrag))

 -- Keep the original Header drag path as a desktop/fallback target.
 own(Header.InputBegan:Connect(function(i)
  if mobile then
   return
  end
 ...
 own(Mini.InputBegan:Connect(function(i)
 ...
 own(Mini.Activated:Connect(function()
  if minimized and not moved then
   setMin(false)
  end
 end))

 own(UIS.InputEnded:Connect(function(i)
  local mouseEnded=
 ...
 own(UIS.InputChanged:Connect(function(i)
  ...
  if not(movingMouse or movingTouch)
   or not(dh or dm)
   or not sm
   or not sp
  then
  ...
  if dm then
   ...
   Mini.Position=
    UDim2.fromOffset(
     math.clamp(bx,28,v.X-28),
     math.clamp(by,28,v.Y-28)
    )
  ...
  -- Keep a useful part of the window reachable, but allow much more movement
  -- than the old desktop clamp. This is especially important on landscape phones.
  local sw=W*Scale.Scale
  local sh=H*Scale.Scale
  ...
  local minY=
   math.min(
    sh/2,
    math.max(34,HEADER*Scale.Scale)
   )
 ...
 own(Close.Activated:Connect(function()window:RequestClose()end))
 return window
end
