-- ScoopHub V2.5 exact-source GUI library (Transparent Header Logo + User Privacy)
...
local Library={Unloaded=false,Build="SCOOPHUB_V2_5_TRANSPARENT_HEADER_LOGO"}
  ...
  function tab:AddUserDashboard(u)
   ...
   local function hover(par,text,pos,size)
    local b=C(N("TextButton",{Text=text,Position=pos,Size=size,BackgroundColor3=Color3.fromRGB(24,22,25),BackgroundTransparency=.08,
     BorderSizePixel=0,AutoButtonColor=false,TextColor3=T.White,Font=T.Font,TextSize=11},par),5);S(b,USER_SOFT,.58,1)
   ...
   local function row(par,t,v,y)
    label(par,t,UDim2.new(0,12,0,y),UDim2.new(.32,-4,0,17),11,T.Muted,T.Body)
    local x=label(par,v,UDim2.new(.32,4,0,y),UDim2.new(.68,-16,0,17),11,T.White,T.Body,Enum.TextXAlignment.Right)
   ...
   local function tog(par,t,d,y,init,cb)
    label(par,t,UDim2.new(0,12,0,y),UDim2.new(1,-75,0,15),11,T.White,T.Font)
    if d and d~="" then
     label(par,d,UDim2.new(0,12,0,y+14),UDim2.new(1,-75,0,14),9,T.Muted,T.Body)
    end
   ...
   label(P,u.Description or "Manage your account, preferences and session.",UDim2.new(0,41,0,24),UDim2.new(1,-45,0,16),11,T.Muted,T.Body)
   ...
   local function pv(t,v,y)
    label(pc,t,UDim2.new(0,114,0,y),UDim2.new(1,-124,0,13),10,T.Muted,T.Body)
    return label(pc,tostring(v),UDim2.new(0,114,0,y+13),UDim2.new(1,-124,0,15),11,T.White,T.Font)
   end
   ...
   label(lc,"WalkSpeed Value",UDim2.new(0,12,0,31),UDim2.new(1,-24,0,14),10,T.Muted,T.Font)
   local wi=C(N("TextBox",{Text=tostring(q.WalkSpeedValue or 16),PlaceholderText="16",ClearTextOnFocus=false,Font=T.Font,TextSize=11,TextColor3=T.White,
